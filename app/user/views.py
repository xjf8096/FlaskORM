...
from .models import User, Permission, Role, Users
...
def query_limit(page, size):
    ...
    """
    # paginate.iter_pages():
    return render_template('user/user.html', users=users, paginate=paginate)
...
def login():
    ...
    if request.method=='get':
        ...
        username=request.value.get('username')
        password=request.value.get('password')
        if username and password:
            users=db.session.query(Users.uid,Users.username,Users.password,Users.email)
            if users:
